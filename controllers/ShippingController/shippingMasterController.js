...
exports.createShippingMaster = async (req, res) => {
    try {
        const { ci_id, shipping_doc_receipt_date_ho, received_from_bank_date_ho, obl_sent_apapa_date, obl_received_date, created_by } = req.body;
...
exports.addcomplianceinShippingMaster = async (req, res) => {
    try {
        const { ci_id, nafdac, son, idec, cria, fast_track_clearing, agency, agent, updated_by } = req.body;
...
exports.updateShippingDetailsByCiId = async (req, res) => {
    ...
    try {
        ...
        if (discharge_terminal !== undefined) updateFields.discharge_terminal = discharge_terminal;
        if (transfer_terminal !== undefined) updateFields.transfer_terminal = transfer_terminal;
        ...
        if (no_of_container !== undefined) updateFields.no_of_container = no_of_container;
        ...
        if (updated) {
            ...
            res.status(200).json({ message: 'Shipping details updated successfully', data: updatedShippingRecord });
        } else {
...
exports.getShippingMasterByCiId = async (req, res) => {
    try {
        ...
        if (!ci_id) {
            return res.status(400).json({ message: 'ci_id query parameter is required' });
        }
...
exports.updateShippingMaster = async (req, res) => {
    try {
        const { id } = req.params;
        const { shipping_doc_receipt_date_ho, received_from_bank_date_ho, obl_sent_apapa_date, obl_received_date } = req.body;
